...
class Fetch:
    ...
    @staticmethod
    def job_detail(board: str) -> Optional[dict]:
        ashby_api = f'https://api.ashbyhq.com/posting-api/job-board/{board}?includeCompensation=true'
        return Get.Json(ashby_api, SESSION)
...
class Write:
    ...
    def Jsonl_1():
        url_objs = Read.Csv()
        jsonl_path = Out_obj.get_filename('ashby1.jsonl')
        for urlobj in url_objs:
            job_detail = Fetch.job_detail(urlobj.board)
    ...
    def Csv():
        job_postings = []
        jsonl1_path = Out_obj.get_filename('ashby1.jsonl')
        ...
        for d in data:
            ...
            for job_data in job_data_all.get("jobs", []):
                ...
                post_date = Extract.publish_date(job_data)
                ...
                for location in locations:
                    ...
                    if len(locations) == 1:
                        reference_id = Get.ReferenceId("ashby", board, job_id)
                    else:
                        further_id = f"_{country}_{city}_{state}"
                        further_id = further_id.replace("_None", "").replace(" ", "").replace(',and', '').replace(',', '_')
                        reference_id = Get.ReferenceId("ashby", board, job_id, further_id)

                    jp_obj = JobPosting()
                    jp_obj.reference_id = reference_id
                    jp_obj.apply_url = job_data.get("applyUrl", "")
                    jp_obj.job_url = job_data.get("jobUrl", "")
                    ...
                    jp_obj.job_type = Get.JobType(job_data.get("employmentType", ""))
                    jp_obj.category = job_data.get("department", "")
                    jp_obj.city = city
                    jp_obj.state = state
                    ...
                    jp_obj.job_description = Extract.description(job_data)

                    job_postings.append(jp_obj)
        
        output_job_path = Out_obj.get_filename('ashby_jobs.csv')
